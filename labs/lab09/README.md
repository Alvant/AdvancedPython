...
[Демо ноутбук](./demo/Coroutines-Into-the-Async.ipynb) про асинхронность.
...
В основной программе (*планировщике задач*, или *оркестраторе*[^scheduler]) должен выполняться цикл.
...
WHILE (есть лепесток хотя бы на одной ромашке AND не нашлась ромашка с итогом "любит"):
...
[^scheduler]: Вообще понятие "оркестратор" обычно встречается в контексте [взаимодействия между компонентами компьютерных систем](https://en.wikipedia.org/wiki/Orchestration_(computing)). Но можно его использовать и более общо, описывая "сущность", которая управляет работой других "сущностей". Для понятия же "планировщик задач" важно бы было ещё определить само понятие "задача". Поэтому в контексте описанного по мотивам лабы задания термины "оркестратор" и "планировщик задач" стоит считать не "строгими", но "передающими суть".
