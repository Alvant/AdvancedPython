...
Надо зайти в Панель управления\Система и безопасность\Система\Дополнительные параметры системы и там кликнуть "Переменные среды", и там будет переменная PATH. В неё надо добавить путь до бинарника с Гитом (куда он установился).
Советы по настройке `PATH`, в частности для работы с Гитом, можно несложно найти.
