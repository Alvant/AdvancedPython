...
Оформите её так, чтобы была возможность запуска из командной строки с передачей параметров.
...
![](./happy_birthday_from_the_cow.png)

Надо доработать файл с кодом, так чтобы его можно было запускать из командной строки *с передачей параметров*: имени человека, его возраста, и пути, по которой должна быть сохранена картинка-поздравление.
Например:
```bash
